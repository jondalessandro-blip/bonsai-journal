        ...
        <div className="space-y-4">
          {selectedGroups.length === 0 ? (
            ...
            calendar.months.map((month) => {
              const mergedTasks = mergeIdenticalTasks(month.tasks);
              const singleGroupSections = groups
                .map((group) => ({
                  heading: group.short_label,
                  groupId: group.group_id,
                  tasks: mergedTasks.filter(
                    (mergedTask) =>
                      mergedTask.groups.length === 1 &&
                      mergedTask.groups[0].group_id === group.group_id,
                  ),
                }))
                .filter((section) => section.tasks.length > 0);
              const severalGroupsTasks = mergedTasks.filter(
                (mergedTask) => mergedTask.groups.length > 1,
              );
              const sections = [
                ...singleGroupSections,
                ...(severalGroupsTasks.length > 0
                  ? [
                      {
                        heading: "Several groups",
                        groupId: "several-groups",
                        tasks: severalGroupsTasks,
                      },
                    ]
                  : []),
              ];
